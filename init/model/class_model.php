	...
	class class_model{
		...
		public function delete_employee($employee_id){
		 ...
		 public function fetchAll_empAttendance(){ 
		  ...
		  public function fetchAll_Lesson(){ 
            $sql = "SELECT * FROM tbl_lesson ORDER BY LessonID DESC";
				$stmt = $this->conn->prepare($sql);
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
		  ...
		  public function add_lesson($LessonChapter, $LessonTitle,$FileLocation,$Category){
			$stmt = $this->conn->prepare("INSERT INTO `tbl_lesson` (`LessonChapter`, `LessonTitle`, `FileLocation`, `Category`) VALUES(?, ?, ?, ?)") or die($this->conn->error);
			 $stmt->bind_param("ssss", $LessonChapter, $LessonTitle, $FileLocation, $Category); //yung ss jn sa bind_param ay meaning nyan ay "string" kapg nakakita kanag i it mins ay "integer" ito ay sa mga id gngamit
			 if($stmt->execute()){
				 $stmt->close();
				 $this->conn->close();
				 return true;
		 ...
		 public function fetchAll_department(){ 
		  ...
		  public function add_department($department_name, $description){
	       $stmt = $this->conn->prepare("INSERT INTO `tbl_department` (`department_name`, `description`) VALUES(?, ?)") or die($this->conn->error);
			$stmt->bind_param("ss", $department_name, $description); //yung ss jn sa bind_param ay meaning nyan ay "string" kapg nakakita kanag i it mins ay "integer" ito ay sa mga id gngamit
			if($stmt->execute()){
				$stmt->close();
				$this->conn->close();
				return true;
	    ...
	    public function fetchindividual_employee($employee_id){ 
         ...
         public function fetchindividual_empAttendance($employee_id){ 
		  ...
		  public function count_numberoflesson(){ 
            $sql = "SELECT COUNT(LessonID) as LessonID FROM tbl_lesson ORDER BY LessonID DESC";
				$stmt = $this->conn->prepare($sql);
				$stmt->execute();
				$result = $stmt->get_result();
		        $data = array();
